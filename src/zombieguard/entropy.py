"""Byte-level entropy utilities for archive payload analysis."""
...
def compute_renyi_entropy(byte_data: bytes, alpha: float = 2.0) -> float:
    """Compute Rényi entropy over a byte sequence for a given alpha."""
    ...
    return float(
        (1.0 / (1.0 - alpha))
        * np.log2(np.sum(probabilities**alpha))
    )
